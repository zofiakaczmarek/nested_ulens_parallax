...
def log_likelihood(params, data=None, delta_sun=None):
    """Log likelihood of point source point lens model with parallax"""
    times, mags, emags = data['times'], data['mags'], data['emags']
    u0, tE, fbl, pi_EE, pi_EN, t0, m0 = params
    u = parallax_trajectory(times, t0, u0, tE, pi_EE, pi_EN, delta_sun=delta_sun)
...
def parallax_trajectory(times, t0, u0, tE, pi_EE, pi_EN, delta_sun=None):
    ...
    return np.hypot(tau + delta_tau, beta + delta_beta)
...
def run_dynesty_sampling(log_likelihood, prior_unit_transform, config, num_params=7):
...
def get_posterior_samples(source_id=None, config=None, size=10000):
    ...
    path = config['paths']['output_directory']
    # order is important
    parameter_names = ['u0', 'tE', 'fbl', 'pi_EE', 'pi_EN', 't0', 'm0']
...
def plot_samples_over_data(posterior=None, data=None, num_samples=100,
    ...
    data_dense = {'times': times_dense,
                  ...
                  'dec': data['dec'],
                  't0': data['t0']}
    delta_sun_dense = annual_parallax(data_dense)
    plt.clf()
    fig, ax = plt.subplots(1,1, figsize=(6,6))
    for sample in range(num_samples):

        u_dense = parallax_trajectory(times_dense,
                                      posterior['t0'][sample],
                                      posterior['u0'][sample],
                                      posterior['tE'][sample],
                                      posterior['pi_EE'][sample],
                                      posterior['pi_EN'][sample],
                                      delta_sun=delta_sun_dense)
